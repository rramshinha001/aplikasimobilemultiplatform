...
/*
  Generated class for the ProductfavoriteProvider provider.
...
export class ProductfavoriteProvider {
  ...
  public insert(product) {
    ...
    let params = [product.id, product.name, product.price,
   product.category, product.image];
    return db.executeSql(sql, params)
    .then((s) => console.log(s+'Product berhasilditambahkan!'))
    .catch(e => console.error('Product gagal ditambahkan:',
   e));
    })
